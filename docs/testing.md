...
Run `pnpm exec playwright install chromium` once before the first local browser test. `pnpm test:e2e` then creates the isolated `ecommerce-e2e` Compose project, applies real migrations, uses real PostgreSQL/Kafka/Temporal/Mailpit, disables JWT validation only in that test project, and enables the fake payment adapter only with `NODE_ENV=test`. Playwright covers public browsing and a complete durable checkout. The runner prints service diagnostics on failure and removes its own volumes in `finally`, including after failure.
