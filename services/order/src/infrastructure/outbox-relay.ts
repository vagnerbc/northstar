...
export function startOutboxRelay(
  ...
  const metrics = createOutboxMetrics('order-service');
  const run = async () => {
    // Claiming through a short transaction lets multiple relay replicas cooperate without
    // publishing the same row concurrently. The lease makes PROCESSING rows recoverable after a crash.
