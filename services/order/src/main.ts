...
const [
  ...
  import('./infrastructure/cart-client.js'),
...
const server = createApp(repository, new HttpCartClient()).listen(config.PORT, '0.0.0.0', () =>
  logger.info({ port: config.PORT }, 'Order service is listening'),
);
